...
org = pd.read_excel(
    "spreadsheets/organizacija.xlsx",
    sheet_name="Opštine u Srbiji",
    names=["region", "okrug", "opstina"],
    usecols=[0, 1, 2],
)

persons = pd.read_excel("spreadsheets/spisak_12_22.xlsx", sheet_name="Spisak").drop(
    ["ID osobe", "Spojnica", "G", "Mesto", "Fotka", "Id groblja"], axis=1
)
persons.columns = "pol,ime,prezime,rodjeno_prezime,srednje_slovo,nadimak,titula,rodjenje,smrt,tip_slike,pismo,simboli,oznake,groblje,opstina,okrug,region".split(
    ","
)
persons = persons.replace(
    {"x": None, "1300 kaplara (van groblja)": "1300 kaplara", "np": None}
)
...
groblja["opstina_id"] = groblja.apply(
    idFromId,
    args=(
        persons,
        "groblje",
...
groblja["name"] = groblja["name"].apply(lambda x: x.title())
opstine["okrug_id"] = opstine.apply(
    idFromId,
    args=(
        org,
        "opstina_id",
...
opstine["name"] = opstine["name"].apply(lambda x: x.title())
okruzi["region_id"] = okruzi.apply(
    idFromId,
    args=(
        org,
        "okrug_id",
...
okruzi["name"] = okruzi["name"].apply(lambda x: x.title())
persons = persons.drop(["opstina", "okrug", "region"], axis=1)
